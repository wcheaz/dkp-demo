...
Ensure Web Worker files required by the CAD viewer are copied to the public static assets directory so they are servable at runtime, and that a committed postinstall script automates this process on every `pnpm install`.
