...
The backend SHALL generate a valid MiTek Exchange XML (`.mxf` Layout format) byte stream containing wall definitions, placements, and structural frame elements derived from active design parameters, ensuring compatibility with MiTek Pamir imports.
...
- **THEN** the generated XML SHALL contain:
  - An `<Mxf>` root element with attributes `version="MXF Version 5.11"`, `originator="dkp-demo agent"`, and namespace declarations `xmlns:xsd` and `xmlns:xsi`
  ...
  - A `<BuildingFrameList>` under `<Building>` with a list of `<BuildingFrame>` elements positioning each generated frame
  - A `<FrameList>` under the root `<Mxf>` tag containing all `<Frame>` definitions, including members, plates, and braces
