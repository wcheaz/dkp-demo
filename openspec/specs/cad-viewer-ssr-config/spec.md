...
Configure Next.js (with Turbopack) to prevent Node.js built-in module imports (`fs`, `path`) from reaching the client bundle, eliminating SSR/bundling crashes caused by Three.js and the CAD viewer library. Ensure the dev server starts cleanly with no worker-file 404 errors.
