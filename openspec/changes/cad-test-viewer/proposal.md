...
Developers and users need a way to preview arbitrary local CAD files interactively using the existing CAD viewer. In addition, the generated DXF files currently use default white/black lines, which do not represent different materials (wood, concrete, steel) realistically. Setting up a dedicated test viewer page and styling DXF layers with material-appropriate colors solves these needs.

## What Changes

- **CAD Test Route**: A new isolated client-side route `/cad-viewer` with a drag-and-drop or file selector to load and display any local CAD file using the existing `<CadViewer>` component.
...
- Adding links or buttons to the `/cad-viewer` page from the main user-facing gallery.
- Persisting uploaded DXF/DWG files in the database or server filesystem.
- Generating 3D DXF coordinates or loading alternative 3D model files (OBJ, GLTF, IFC) in this change.

## Capabilities

### New Capabilities
- `cad-test-viewer`: Isolated client-side route at `/cad-viewer` for drag-and-drop local CAD file rendering.
...
- **Frontend Pages**: New route page `src/app/cad-viewer/page.tsx`.
