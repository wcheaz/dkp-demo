...
- [x] 1.1 Install `@mlightcad/cad-simple-viewer` via `pnpm add @mlightcad/cad-simple-viewer`. Verify `pnpm install` completes with no peer dependency warnings related to React, Vue, or other frameworks. Run `pnpm build` to confirm the Next.js production build still succeeds with the new dependency in `node_modules`.
