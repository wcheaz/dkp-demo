...
**Reason**: The `add_design_entry` tool was replaced by `generate_design` in a prior change. The old requirement referencing `add_design_entry` is no longer accurate.
**Migration**: The backend now has `generate_design` referenced in the system prompt instead. The `DesignEntry` model and `designs` field on `YourState` remain active.
