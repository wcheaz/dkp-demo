...
Modifies the `modify_design_entry` frontend tool to accept dynamically-served image URLs in addition to the existing hardcoded SVG filename whitelist.
...
#### Scenario: Modify image only with image_name (unchanged behavior)
...
#### Scenario: Modify prompt text only (unchanged behavior)
...
#### Scenario: Modify both image and prompt text (unchanged behavior)
...
#### Scenario: Design ID not found (unchanged behavior)
...
#### Scenario: Invalid image name (unchanged behavior)
...
### Requirement: Design ID displayed in UI (unchanged)
