...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 estimate_price_real.py <path_to_mxf_file> [timber_rate_eur_m3] [fabrication_markup] [setup_cost]")
        print("\nNote: This script automatically substitutes GNA20 placeholder plate prices with realistic Slovak catalog prices.")
        print("\nDefaults:")
    ...
    try:
        # Run with substitute_real_prices=True by default!
        results = estimate_mxf_materials_and_price(
            ...
            substitute_real_prices=True
        )
        
        print(f"\n=== Cost Estimate (with Plate Price Substitution) for {mxf_path} ===")
        ...
        print(f"Total Plates Cost (Substituted): {results['plates_cost_eur']:.2f} €")
        ...
        for item in results["line_items"]:
            ...
            print(f"    Plates (Substituted): {item['plates_cost_eur']:.2f} €")
