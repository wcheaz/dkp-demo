...
class TestValidRequest:
    @pytest.mark.anyio
    async def test_returns_200_with_ifc_content(self, client):
        resp = await client.post("/api/ifc/generate", json=_valid_payload())
        assert resp.status_code == 200
        assert len(resp.content) > 0
        body = resp.content.decode("utf-8", errors="replace")
        assert "FILE_SCHEMA(('IFC2X3'))" in body

    @pytest.mark.anyio
    async def test_content_type_is_ifc(self, client):
        resp = await client.post("/api/ifc/generate", json=_valid_payload())
        assert resp.status_code == 200
        assert "application/ifc" in resp.headers.get("content-type", "")
    ...
    async def test_content_disposition_header(self, client):
        ...
        assert 'attachment; filename="design.ifc"' in cd
...
class TestMissingRoofType:
    ...
    async def test_missing_roof_type_returns_400(self, client):
        ...
        resp = await client.post("/api/ifc/generate", json=payload)
...
class TestInvalidRoofType:
    ...
    async def test_invalid_roof_type_returns_400(self, client):
        resp = await client.post(
            "/api/ifc/generate", json=_valid_payload(roofType="Dome")
        )
        assert resp.status_code == 400
        data = resp.json()
        assert "error" in data
