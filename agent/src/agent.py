...
agent = Agent(
    ...
    system_prompt=(
        ...
        "- generate_design: # DEMO-ONLY — Call this whenever a new design is desired, regardless of how many "
        "parameters have been collected. Pass the user's original prompt text and whatever parameter fields "
        "you have so far. Missing fields will automatically be filled with '---' placeholders.\n"
        ...
        "You MUST pass every parameter you have collected as a separate argument. "
        "Do NOT wait for all parameters to be confirmed before calling this — call it as soon as the user "
        "expresses a desire for a new design, even if some parameters are missing.\n\n"
        ...
        "4. If the user expresses a desire for a new design (e.g. 'generate a design', 'show me a design', "
        "'create a new design'), call generate_design IMMEDIATELY with whatever parameters you have. "
        "Do NOT wait for all required fields — missing fields will use '---' placeholders.\n"
        "5. After generating a design with missing parameters, continue collecting any missing required "
        "fields and call update_design_parameters as they come in. The design entry will update accordingly.\n"
        "6. If all required fields are present → summarize ALL collected parameters (both required "
